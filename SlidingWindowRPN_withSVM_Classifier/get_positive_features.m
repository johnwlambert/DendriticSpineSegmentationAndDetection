...
function features_pos = get_positive_features(train_path_pos, cell_size, window_size, block_size, nbins)
...
for ii = 1:num_images
    img = im2single(imread(strcat(train_path_pos, '/', image_files(ii).name)));
    features_pos(ii,:) = reshape(computeHOGFeatures(img, cell_size, block_size, nbins), 1, D);
end
